...
output_file = open('sample2.csv', 'w') 
   ...
   with open(os.path.join(os.getcwd(), filename), 'r') as input_file:
       for json_data in input_file.readlines():
           ...
           if line['event_type'] == 'asset-searched' or line['event_type'] == 'image-replaced':
               if line['event_type'] == 'asset-searched':
...
'''Remove duplicates'''
col_names = ['client_event_time','project-id','search-term','search-type']
df = pd.read_csv('sample2.csv', usecols= col_names).drop_duplicates(keep='first').reset_index()
